...
int main (int argc, char *argv[]) {
    ...
    switch(fork()) {
      ...
      case 0:
        ...
        num_bytes_child = read(pipeFds[0], childBuffer, sizeof(childBuffer));   // Read file contents from upstream pipe into childBuffer
        close(pipeFds[0]);                                                        // close reading upstream pipe when we're done with it

        targetDesc = open(dstFile, O_CREAT | O_WRONLY);                                  // Open a file for writing, create file descriptor.
      ...
      default:
        ...
        fileInDesc = open(srcFile, O_RDONLY);                       // Read file into file descriptor
        num_bytes = read(fileInDesc, buffer, sizeof(buffer));   // Get number bytes to read
